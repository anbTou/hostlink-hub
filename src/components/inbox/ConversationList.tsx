    ...
    <div className="h-full flex flex-col border-r border-border">
      <div className="p-4 border-b border-border">
        ...
        <div className="flex space-x-2 overflow-x-auto py-1 mb-3 scrollbar-hide">
          ...
          <Button 
            variant={statusFilter === "followup" ? "default" : "outline"} 
            size="sm"
          ...
          >
            Follow-up
            {followupCount > 0 && (
              <Badge variant="secondary" className="ml-1">{followupCount}</Badge>
            )}
          </Button>
          <Button 
          ...
          >
            Done
            {doneCount > 0 && (
              <Badge variant="secondary" className="ml-1">{doneCount}</Badge>
            )}
          </Button>
        </div>
        ...
        <div className="flex space-x-2 overflow-x-auto py-1 scrollbar-hide">
          <Button 
            variant={sourceFilters.includes("email") ? "default" : "outline"} 
            size="sm"
            onClick={() => toggleSourceFilter("email")}
            className="flex gap-2 items-center"
          >
            <Mail className="h-3.5 w-3.5" />
            <span>Email</span>
            {sourceCounts.email > 0 && (
              <Badge variant="secondary" className="ml-1">{sourceCounts.email}</Badge>
            )}
          </Button>
          <Button 
            variant={sourceFilters.includes("airbnb") ? "default" : "outline"} 
            size="sm"
          ...
          >
            ...
            <span>Airbnb</span>
            {sourceCounts.airbnb > 0 && (
              <Badge variant="secondary" className="ml-1">{sourceCounts.airbnb}</Badge>
            )}
          </Button>
          <Button 
            variant={sourceFilters.includes("booking") ? "default" : "outline"} 
            size="sm"
            onClick={() => toggleSourceFilter("booking")}
            className="flex gap-2 items-center"
          >
            <svg className="h-3.5 w-3.5 fill-current" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
            ...
            {sourceCounts.booking > 0 && (
              <Badge variant="secondary" className="ml-1">{sourceCounts.booking}</Badge>
            )}
          </Button>
          <Button 
            variant={sourceFilters.includes("vrbo") ? "default" : "outline"} 
            size="sm"
            onClick={() => toggleSourceFilter("vrbo")}
            className="flex gap-2 items-center"
          >
            <svg className="h-3.5 w-3.5 fill-current" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
            ...
            <span>VRBO</span>
            {sourceCounts.vrbo > 0 && (
              <Badge variant="secondary" className="ml-1">{sourceCounts.vrbo}</Badge>
            )}
          </Button>
          <Button 
            variant={sourceFilters.includes("whatsapp") ? "default" : "outline"} 
            size="sm"
          ...
          >
            ...
            {sourceCounts.whatsapp > 0 && (
              <Badge variant="secondary" className="ml-1">{sourceCounts.whatsapp}</Badge>
            )}
          </Button>
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto">
        {Object.keys(groupedConversations).length === 0 ? (
          ...
          <div>
            {Object.entries(groupedConversations).map(([dayLabel, dayConversations]) => (
              <div key={dayLabel}>
...
function SourceBadge({ source }: { source: ConversationSource }) {
  ...
  return (
    <span className={`text-xs px-1.5 py-0.5 rounded-sm flex items-center ${config.color}`}>
      {config.icon}
      {config.label}
    </span>
  );
